...
function getUsernameColor(_username) {
  ...
  $createGame.click(function () {                               sendGame();                 });
  ...
  $buyDevCard.click(function(){                                 buyDevCard();                   });
  $openTradingCard.click(function(){                            showTradePanel();               });
  $openTradingPlayers.click(function(){                         showTradeWithPlayersPanel();    });
  $playKnight.click(function(){                                 playKnight();                   });
  $playRoadBuilding.click(function(){                           playRoadBuilding();             });
  $playVictoryPoint.click(function(){                           playVictoryPoint();             });
  $playMonopoly.click(function(){                               playMonopoly();                 });
  $playYearOfPlenty.click(function(){                           playYearOfPlenty();             });
  $rollDice.click(function () {                                 rollDice();                     });
  $endTrading.click(function () {                               endTrading();                   });
  ...
  /* Client Controllers */
    ...
    socket.on('myDeck', function (data) {
        ...
        //addChatGameMessage({ username: "", message: "You received your deck." });
        if (canvas) {
    ...
    });// End Socket 'SEND_CHOOSEN_RESOURCE_OF_MONOPOLY'
    ...
    socket.on('gameData', function (data) {
        ...
        if (data.currentTrades) console.log(JSON.stringify(data.currentTrades));
        ...
        if (gameAction.todo == "THIEF" && playerDeck.resourceCount > playerDeck.resourceLimit) {
            ...
            if (isMyTurn()) {
                ...
                if (gameAction.todo == "SETTLEMENT" || gameAction.todo == "ROAD" || gameAction.todo == "CITY") {
 ...
 ********************/ 
    ...
    * */
     ...
     * 
     */
    function canPlaceRoad(road) {
        //console.log("Can the road be placed? ");// LOG
        // SPECIAL TURN
        if (isSpecialTurn()) {
            //console.log("Can the road be placed in this special turn?");// LOG
            var isolatedBuild = catan.world.nodes.find(function (ele) {
    ...
    * */
    ...
    function nodeIsAtRight(i, j) {
        if (i == -3 || i == 1 || i == 5) return j % 2 == 0;
        else return j % 2 != 0;
    }

    function isSpecialTurn() {
        return catan.currentTurn.turn <= catan.rules.specialTurnCount;
    }
